    ...
    return {
        ...
        printPO(order) {
            this.lastTx = {
                ...
                cart: order.items.map(i => ({ name: i.product ? i.product.name : i.package.name, qty: i.qty, price: i.price })),
        ...
        reprintVendorNota(notaDetail) {
            const reconstructedCart = notaDetail.cart.map(c => {
                ...
                return { name: prod ? prod.name : 'Produk', qty: c.qty || c[1], buyPrice: c.buyPrice || c[2], returQty: c.returQty || 0 };
            });
        ...
        reprintTransaction(tx) {
            let reconstructedCart = [];
            let isPkg = tx.invoice && tx.invoice.startsWith('PKT-');

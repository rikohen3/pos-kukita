...
// 2. ENDPOINT: TRANSAKSI PENJUALAN
app.post('/api/checkout', async (req, res) => {
  ...
  try {
    const result = await prisma.$transaction(async (tx) => {
      ...
      const sale = await tx.sale.create({ data: { invoice: invoiceNumber, totalAmount, paymentMethod, cashReceived: cashReceived || 0 } });
...
app.put('/api/products/:id/stock-v2', async (req, res) => {
  ...
  try {
    ...
    if (diff > 0) { await prisma.stockHistory.create({ data: { productId, productName: product.name, qtyAdded: diff, newTotal: newStock } }); }
    res.json({ success: true, data: updated });
  } catch (error) { res.status(500).json({ success: false, error: error.message }); }
});

// 5. ENDPOINT: TAMBAH KUE BARU
...
// 6. ENDPOINT: PENGATURAN PIN PEMILIK
app.post('/api/settings/verify-pin', async (req, res) => {
  const { pin } = req.body;
  try {
    const setting = await prisma.setting.findFirst();
    if (!setting) return res.json({ success: pin === '123456' });
    res.json({ success: setting.adminPin === pin });
  } catch (error) { res.status(500).json({ success: false, error: error.message }); }
});
...
app.put('/api/settings/update-pin', async (req, res) => {
  ...
  try {
    ...
    if (!setting) {
      ...
      await prisma.setting.create({ data: { adminPin: newPin } });
      return res.json({ success: true });
    }
    if (setting.adminPin !== oldPin) return res.json({ success: false, message: 'PIN Lama salah' });
    await prisma.setting.update({ where: { id: setting.id }, data: { adminPin: newPin } });
...
app.get('/api/reports', async (req, res) => {
  ...
  try {
    const sales = await prisma.sale.findMany({ where: dateFilter, orderBy: { createdAt: 'desc' }, include: { items: { include: { product: true, package: true } } } });
    ...
    // PERBAIKAN BUG FATAL VERCEL: e.description bisa bernilai null
...
app.get('/api/reports/items', async (req, res) => {
  ...
  try {
    ...
    const catatBarang = (produk, qtyTerjual, omset, label) => {
      if (!produk) return; // Pengaman jika ada barang terhapus
      const supName = produk.supplier ? produk.supplier.name : 'Unknown';
      const key = produk.id + '_' + label;
      if (!itemSummary[key]) { itemSummary[key] = { id: key, name: produk.name + (label === 'Bijian' ? '' : ` (${label})`), supplier: supName, qtySold: 0, totalSales: 0 }; }
...
// 10. ENDPOINT: HAPUS STRUK
app.delete('/api/transactions/:invoice', async (req, res) => {
  const invoiceNo = req.params.invoice;
  try {
    const sale = await prisma.sale.findUnique({ where: { invoice: invoiceNo }, include: { items: { include: { package: { include: { items: true } } } } } });
    ...
    await prisma.$transaction(async (tx) => {
      for (const item of sale.items) {
        ...
        else if (item.packageId && item.package) { for (const pkgItem of item.package.items) { await tx.product.update({ where: { id: pkgItem.productId }, data: { stock: { increment: pkgItem.qty * item.qty } } }); } }
      }
...
// WAJIB UNTUK VERCEL (TIDAK BOLEH PAKAI APP.LISTEN)
module.exports = app;